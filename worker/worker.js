 ...
 *
 * Self-service sign-up uses Twilio (SMS or WhatsApp) or MSG91 to send
 * a verification code.  Configure credentials in admin Settings.
 * =====================================================================*/
...
export default {
  async fetch(req, env, ctx) {
    ...
    try {
      ...
      if (req.method === 'GET' && path === '/api/menu')     return getMenu(url, env);
      ...
      if (req.method === 'POST' && path === '/api/signup')        return signupRequest(req, env);
      if (req.method === 'POST' && path === '/api/signup/verify') return signupVerify(req, env);
...
async function getMenu(reqUrl, env) {
  ...
  for (const p of prods) {
    // Build absolute image URL from R2 key if no explicit URL saved
    if (!p.image_url && p.image_key) {
      p.image_url = publicBase
        ? publicBase.replace(/\/$/, '') + '/' + p.image_key.replace(/^\//, '')
        : workerOrigin + '/img/' + p.image_key.replace(/^\//, '');
    }
    // Fix up any relative /img/ URLs saved previously
    if (p.image_url && p.image_url.startsWith('/img/')) {
      p.image_url = workerOrigin + p.image_url;
    }
  }
...
// Step 1: customer submits phone + name → we send verification code
async function signupRequest(req, env) {
  ...
  if (enabledRow && enabledRow.value === '0') return err('Sign-up is currently disabled', 403);

  // Check if phone is already registered
  const existing = await env.DB.prepare("SELECT id, pin_hash FROM customers WHERE phone = ?").bind(phone).first();
  if (existing && existing.pin_hash) return err('This phone is already registered. Use sign-in.', 409);

  // Rate limit: max 3 pending codes per phone in the last 10 minutes
  const recent = await env.DB.prepare(
    "SELECT COUNT(*) as cnt FROM pending_signups WHERE phone = ? AND created_at > datetime('now', '-10 minutes')"
  ).bind(phone).first();
  if (recent && recent.cnt >= 3) return err('Too many attempts. Please wait 10 minutes.', 429);
  ...
  const message = `${storeName}: Your sign-up code is ${code}. Valid for 10 minutes.`;

  const sendResult = await sendVerificationMessage(env, phone, message);
  if (!sendResult.ok) {
    return err('Could not send verification message: ' + sendResult.error, 500);
  }

  return json({ ok: true, message: 'Verification code sent to ' + phone.slice(-4).padStart(phone.length, '*') });
}

// Step 2: customer submits phone + code + new PIN → we create the account
async function signupVerify(req, env) {
  const body = await req.json().catch(() => null);
  if (!body) return err('Invalid JSON');
  const phone  = normalizePhone(body.phone);
  ...
  const name   = (body.name || '').trim();
  if (!phone || !code || !pin) return err('phone, code and pin required');
  if (pin.length < 4) return err('PIN must be at least 4 characters');

  // Find the latest pending signup for this phone
  const pending = await env.DB.prepare(
    "SELECT id, code, attempts, expires_at, name FROM pending_signups WHERE phone = ? ORDER BY created_at DESC LIMIT 1"
  ).bind(phone).first();

  if (!pending) return err('No sign-up in progress for this phone. Request a code first.', 404);

  // Check expiry
  const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
  if (pending.expires_at < now) {
    await env.DB.prepare("DELETE FROM pending_signups WHERE phone = ?").bind(phone).run();
    return err('Code expired. Please request a new one.', 403);
  }

  // Check attempts (max 5)
  if (pending.attempts >= 5) {
    await env.DB.prepare("DELETE FROM pending_signups WHERE phone = ?").bind(phone).run();
    return err('Too many wrong attempts. Please request a new code.', 403);
  }

  // Verify code
  if (!safeEqual(code, pending.code)) {
    await env.DB.prepare(
      "UPDATE pending_signups SET attempts = attempts + 1 WHERE id = ?"
    ).bind(pending.id).run();
    return err('Invalid code. ' + (4 - pending.attempts) + ' attempts remaining.', 403);
  }
  ...
  // Log it
  await env.DB.prepare(
    "INSERT INTO admin_log (action, target, detail) VALUES ('self-signup', ?, 'verified and PIN set')"
  ).bind(phone).run();
...
// ---------------------------------------------------------------
// Send verification message via Twilio (SMS or WhatsApp) or MSG91
// ---------------------------------------------------------------
async function sendVerificationMessage(env, phone, message) {
  // Read provider config from settings
  const settings = {};
  const rows = (await env.DB.prepare(
    "SELECT key, value FROM settings WHERE key IN ('sms_provider','twilio_account_sid','twilio_auth_token','twilio_from_number','twilio_use_whatsapp','msg91_auth_key','msg91_sender_id','msg91_template_id')"
  ).all()).results;
  ...
  if (provider === 'twilio') {
    return sendViaTwilio(settings, phone, message);
  } else if (provider === 'msg91') {
    return sendViaMSG91(settings, phone, message);
  } else {
    return { ok: false, error: 'No SMS/WhatsApp provider configured. Set sms_provider in admin Settings.' };
  }
}

async function sendViaTwilio(s, phone, message) {
  ...
  const useWA = s.twilio_use_whatsapp === '1';
  const to   = useWA ? 'whatsapp:' + phone : phone;
  const fromN = useWA && !from.startsWith('whatsapp:') ? 'whatsapp:' + from : from;

  const url = `https://api.twilio.com/2010-04-01/Accounts/${sid}/Messages.json`;
  const body = new URLSearchParams({ To: to, From: fromN, Body: message });
  ...
  try {
    ...
    if (r.ok || j.sid) return { ok: true };
    return { ok: false, error: j.message || ('Twilio HTTP ' + r.status) };
  } catch (e) {
    return { ok: false, error: 'Twilio request failed: ' + e.message };
  }
}

async function sendViaMSG91(s, phone, message) {
  const authKey    = s.msg91_auth_key;
  const senderId   = s.msg91_sender_id || 'VERIFY';
  const templateId = s.msg91_template_id;
  if (!authKey) return { ok: false, error: 'MSG91 auth key not configured' };

  // Extract code from message for the template variable
  ...
  try {
    ...
    return { ok: false, error: j.message || ('MSG91 HTTP ' + r.status) };
  } catch (e) {
    return { ok: false, error: 'MSG91 request failed: ' + e.message };
  }
}
...
async function adminImageUpload(req, env) {
  ...
  const baseRow = await env.DB.prepare("SELECT value FROM settings WHERE key='images_public_base'").first();
  const publicBase = (baseRow && baseRow.value) || env.IMAGES_PUBLIC_BASE || '';
  // Use Worker's own origin as fallback so image URLs are always absolute
  const workerOrigin = url.origin;
  const publicUrl = publicBase
    ? publicBase.replace(/\/$/, '') + '/' + key
    : workerOrigin + `/img/${key}`;
